/**
 * 화면 요소를 PNG로 캡처한다. html-to-image는 쓸 때만 동적으로 불러온다.
 *
 ...
 */
async function toPngDataUrl(element: HTMLElement): Promise<string> {
...
/**
 * 같은 캡처를 파일로 돌려준다 — 카카오톡 공유는 이미지 "파일"을 카카오 서버에 올린 뒤
 * 그 URL을 템플릿에 넣는 구조라(src/lib/kakao.ts 참고), 다운로드용 data URL로는 안 된다.
 */
export async function captureElementAsFile(

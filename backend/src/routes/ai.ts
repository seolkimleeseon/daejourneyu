...
router.post("/course-suggestion", asyncHandler(async (req, res) => {
  ...
  const responseSchema = {
    ...
    properties: {
      ...
      summary: {
        type: Type.STRING,
        description:
          "responseType이 course일 때만 채운다. 이 코스를 왜 이렇게 짰는지 1~2문장(이동이 짧은 이유, 반려동물을 배려한 점, 장소 구성). 방문 순서는 서버가 동선에 맞게 다시 정렬하므로 '먼저·다음·마무리' 같은 순서 표현은 쓰지 않는다. 존댓말, 120자 이내.",
      },
      days: {
        type: Type.ARRAY,
  ...
  const systemInstruction =
    ...
    "summary에는 왜 이렇게 짰는지를 근거(이동이 짧다, 반려견에게 맞는 조건 등)와 함께 1~2문장으로 쓰되, 방문 순서는 서버가 다시 정렬하니 순서를 단정하는 표현은 빼. " +
